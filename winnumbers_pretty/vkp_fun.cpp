...
int show(struct posib *p) //Just for Fucking Windows
{
	int ret;
...
int situation(struct posib *p, char gr) //devuelve la situacion para analizar el numero
{
	char ret=-1;
	if(gr==3)
	{
...
/* Replaces because the printfs  method
int ask(struct posib *p)
{
	...
	if(p->goods != 3 && p->goods != 4)
	{
		...
		{
			printf("Wrong entered value, trys again:\n");
			p->goods=check("GOODS!...");
			p->regs=check("REGS!...");
		}	
	}
...
int notanymoreh(struct posib *psource, struct hidden *pdest)
{
	char i;
	int ret=0;	
	 		 ...
	 		 if(cnt == 2)
		 	 {
				...
				for(i=0;i<4;i++)
				{
				  if(i>=2)
				  {
					...
					element=notanymoreh(&p[toplay], chanceless) -1;
	...
	else if(flag== GR4_PRTY75_25)
	{
		...
		else if(simplchange == 1)
		{
			...
			if(p[cnt].prty == 0.50)
			{
				...
				{
					element=notanymoreh(&p[cnt], chanceless) -1;
					pr->nums[element].pos=-1;
					pr->nums[element].trys=-1;
...
int vkp_debug(struct posib *p, struct prob *pr, struct hidden *secret, 
			  struct hidden *temp, struct hidden *chanceless, struct hidden *justout,
			  char gr, char cnt, char jgds,	char autodebug,	char fibyn)
{
